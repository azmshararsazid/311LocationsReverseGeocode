...
**Modify these lines in rpython script as needed:**

- **Line 149 (Input CSV):** Change to `hl = pd.read_csv(r"your_file.csv", encoding="cp1252")`. Use format `r"C:\Users\USER\filename.csv"` for full paths.
- **Line 154 (Latitude column):** If your column isn't named `Latitude`, change to `lat = row["Your_Column_Name"]`
- **Line 155 (Longitude column):** If your column isn't named `Longitude`, change to `lon = row["Your_Column_Name"]`
- **Line 168 (Output filename):** Change to `out_path = "your_output.csv"`
- **Line 149 (Encoding):** If CSV is from Google Sheets or Mac, change `"cp1252"` to `"utf-8"`

## Troubleshooting

- **`KeyError: 'Latitude'` or `'Longitude'`** — Column name doesn't match (case-sensitive). Check line 154-155.
